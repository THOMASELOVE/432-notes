...
```{r}
model_parameters(m1, exponentiate = TRUE, ci = 0.90)
```
...
m2 <- stan_glm((smoker == 1) ~ age + sex + totchol + factor(educ), 
               data = smalldat, refresh = 0, family = binomial())
```
...
```{r}
model_parameters(m2, exponentiate = TRUE, ci = 0.90)
```
...
Again, these are somewhat close to what we obtained from least squares estimation.
...
So we conclude that the `age` effect has a 95% chance of falling within the [-0.11, -0.02] range. We can also exponentiate here, so as to provide the result in terms of an odds ratio.
...
## check_model() results for models m1 and m2
