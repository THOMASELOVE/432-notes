...
The `pd` estimate helps us understand whether each effect is positive or negative. For instance, regarding `age`, we see the proportion of the posterior that is positive, no matter what the "size" of the effect is, will be as follows. 
...
So we see that the effect of `age` is positive with a probability of 99.5%, and this is called the **probability of direction** and abbreviated **pd**.
...
The probability that each effect is positive is summarized in the `pd` column of our `describe_posterior()` results.
