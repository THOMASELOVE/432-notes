...
We can use the `extract_eq()` function from the **equatiomatic** package to display the model attractively, but I won't show that output here, because it interferes with my ability to create a PDF of this work. Instead, I'll just show the code.
...
Again, I'll show only the code to present the equation, so I can produce a PDF version of these notes.
